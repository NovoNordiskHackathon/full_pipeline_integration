// PTD Generator with backend integration
...
class PTDGenerator {
  constructor() {
    ...
    this.API_BASE_URL = "http://127.0.0.1:5000"; // Update if needed
    // Keep double-slash route for backward compatibility
    this.RUN_PIPELINE_URL = `${this.API_BASE_URL}//run_pipeline`;
  ...
  showFileTypeModal(file) {
    ...
    modal.innerHTML = `
      <div class="modal-content">
        ...
        <div class="modal-buttons">
          <button onclick="ptdGenerator.assignFileType('protocol')" class="modal-btn">
            <i class="fas fa-file-alt"></i> Protocol Document
          </button>
          <button onclick="ptdGenerator.assignFileType('crf')" class="modal-btn">
    ...
    `;
    // Temporarily store the single file on the instance to assign later
    this._singleDroppedFile = file;
    document.body.appendChild(modal);
  }

  assignFileType(type) {
    const file = this._singleDroppedFile;
    if (!file) return;
    if (type === 'protocol') {
  ...
  async generatePTD() {
    ...
    try {
      // Call the backend API to upload files (mirrors prior integration)
      await this.callBackendAPI();

      // For now, simulate internal processing (conversion/structuring) to mirror old behavior
  ...
  async callBackendAPI() {
    ...
    const uploadResponse = await fetch(this.RUN_PIPELINE_URL, {
      method: 'POST',
      body: formData
    });
  ...
  async simulateProcessing() {
    const steps = [
      { name: 'Uploading files...', duration: 1200 },
      { name: 'Analyzing document structure...', duration: 1600 },
      { name: 'Extracting data...', duration: 1400 },
      { name: 'Generating PTD...', duration: 1200 },
      { name: 'Finalizing output...', duration: 800 }
    ];
  ...
  showOutputSection() {
    ...
    // If the backend provides a real download URL later, plug it here
    if (this.processingResult && this.processingResult.download_url) {
      downloadLink.href = `${this.API_BASE_URL}${this.processingResult.download_url}`;
    } else {
      // Sample data URL placeholder
