...
def extract_text_and_tables_from_pdf(input_pdf_path: str, output_zip_path: str):
    try:
        logging.info(f"Attempting to extract text and tables with enhanced structure from {input_pdf_path}")
        ...
        # Define extraction parameters for text and tables WITH table structure renditions
        ...
        logging.info(f"✅ Successfully extracted text and tables with structure. Output saved to: {output_zip_path}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Extract text and tables with structure from a PDF using Adobe PDF Services API.")
    parser.add_argument("input_pdf", type=str, help="Path to the input PDF file")
    parser.add_argument("-o", "--output", type=str, default="extractTextTableInfoFromPDF.zip",
                        help="Output ZIP file path (default: extractTextTableInfoFromPDF.zip)")
    args = parser.parse_args()

    extract_text_and_tables_from_pdf(args.input_pdf, args.output)
